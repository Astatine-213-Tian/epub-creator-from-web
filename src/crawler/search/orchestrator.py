...
def load_search_module(parser_name: str) -> SearchModule | None:
    try:
        return importlib.import_module(f"src.crawler.providers.{parser_name}.search")
    except ModuleNotFoundError as exc:
        if exc.name == f"src.crawler.providers.{parser_name}.search":
            return None
        raise
...
def search_all(
    ...
    *,
    ...
    browser_jobs = [
        (order, name, module)
        for order, name, module in jobs
        if _uses_browser_for_search(module)
    ]
    thread_jobs = [
        (order, name, module)
        for order, name, module in jobs
        if not _uses_browser_for_search(module)
    ]
    ...
    def record_search_outcome(outcome: SearchJobOutcome) -> None:
        ...
        if debug:
            ...
            search_counter.advance(
                detail=f"{outcome.name}: {len(provider_results)} result(s)"
            )
...
async def _run_search_jobs(
    ...
    *,
    ...
    thread_executor = (
        ThreadPoolExecutor(max_workers=len(thread_jobs)) if thread_jobs else None
    )
    browser = None
    try:
...
async def _run_thread_search(
    ...
    try:
        results = await loop.run_in_executor(
            executor,
            partial(
                module.search_books, query, limit=limit_per_provider, author=author
            ),
        )
...
async def _start_search_browser() -> Any:
    from src.crawler.fetch.browser import start_browser
...
def build_previews(
    ...
    *,
    ...
    if debug:
        browser_count = sum(
            1 for job in preview_jobs if _uses_browser_for_preview(job.module)
        )
        progress.info(
    ...
    def record_preview_outcome(outcome: PreviewJobOutcome) -> None:
        ...
        if outcome.error is not None:
            progress.warning(
                f"preview failed for {result.parser} {result.url}: {outcome.error}"
            )
            if not debug:
        ...
        count = (
            preview.chapter_count if preview.chapter_count is not None else "unknown"
        )
        if debug:
            progress.info(
                f"Preview ready: {preview.parser} | {preview.title} ({count} chapter(s))"
            )
        else:
    ...
    if debug:
        ...
        preview_counter.finish(
            f"Previews complete: {len(ranked_previews)} usable preview(s)"
        )
    ...
    try:
        if thread_executor is not None:
            for job in thread_jobs:
                if debug:
                    progress.info(
                        f"Preview {job.index}/{len(jobs)}: {job.result.parser} | {job.result.title}"
                    )
        ...
        if browser_jobs:
            ...
            for job in browser_jobs:
                ...
                tasks.append(
                    asyncio.create_task(
                        _with_preview_timeout(
                            _run_browser_tab_preview(job, browser), job=job
                        )
                    )
                )
...
def dedupe_previews(previews: list[BookPreview]) -> list[BookPreview]:
    ...
    for preview in previews:
        ...
        if current is None or _preview_url_rank(preview.url) < _preview_url_rank(
            current.url
        ):
...
def choose_preview(
    previews: list[BookPreview], *, first: bool = False
) -> BookPreview | None:
    if not previews:
        return None
    if first:
...
def fake_menu_previews() -> list[BookPreview]:
    return [
        BookPreview(
            ...
            last_chapters=(
                "第175章 2026.02.28番外（上）",
                "第176章 2026.03.01番外（下）",
            ),
        ),
        BookPreview(
            parser="mgsf",
    ...
    try:
        ...
        while True:
            ...
            if key in {"UP", "k"}:
                ...
                message = (
                    "Opened source URL in browser."
                    if opened
                    else "Could not open browser."
                )
                needs_render = True
            elif key in {"q", "Q"}:
                print()
...
def _render_preview_menu(
    previews: list[BookPreview], selected: int, *, message: str = ""
) -> None:
    ...
    print(
        _trim_for_terminal(
            "    #  Provider     Chapters       Title                              Author",
            width,
        )
    )
...
def _print_selected_preview(preview: BookPreview, width: int) -> None:
    ...
    if preview.first_chapters:
        print(
            _trim_for_terminal(f"First:    {' / '.join(preview.first_chapters)}", width)
        )
    if preview.last_chapters:
        print(
            _trim_for_terminal(f"Last:     {' / '.join(preview.last_chapters)}", width)
        )
