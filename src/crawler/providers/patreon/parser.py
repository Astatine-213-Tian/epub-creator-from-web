...
from src.content.html import is_scene_break_text, render_scene_break
...
class PatreonFetcher:
    ...
    async def get_json(self, url: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise RuntimeError(
                f"Patreon API returned non-JSON for {url}: {sample}"
            ) from exc
    ...
    async def get_bytes(self, url: str) -> tuple[bytes, str]:
        ...
        if not payload.get("ok"):
            ...
            raise RuntimeError(
                f"Patreon fetch failed with HTTP {status} {status_text}: {url}"
            )
...
def _parse_collection(
    ...
    for index, post_id in enumerate(ids):
        ...
        if post_attrs and not _is_published(post_attrs):
            continue
        title = _clean_title(
            str(post_attrs.get("title") or ""), fallback=f"Chapter {index + 1:03d}"
        )
...
def _post_ref_from_api_post(post: dict[str, Any], *, order: int) -> PostRef | None:
    ...
    if attrs.get("current_user_can_view") is False:
        return None
    title = _clean_title(
        str(attrs.get("title") or ""), fallback=f"Chapter {order + 1:03d}"
    )
    return PostRef(
        post_id=post_id,
        title=title,
...
def _extract_post_paragraphs(attrs: dict[str, Any]) -> list[str]:
    for candidate in _content_candidates(attrs):
        if isinstance(candidate, str):
            ...
            paragraphs = []
        cleaned = [
            _normalize_text(item) for item in paragraphs if _normalize_text(item)
        ]
        if cleaned:
            return cleaned
    return []
...
def _flatten_comment(
    ...
    *,
    ...
    commenter = ((comment.get("relationships") or {}).get("commenter") or {}).get(
        "data"
    ) or {}
...
async def fetch_post_comments_authenticated(
    ...
    while url:
        ...
        for item in data.get("data") or []:
            ...
            reply_refs = ((item.get("relationships") or {}).get("replies") or {}).get(
                "data"
            ) or []
...
def _chapter_from_post(post_ref: PostRef, data: dict[str, Any]) -> Chapter:
    ...
    if not paragraphs:
        ...
        raise RuntimeError(
            f"could not find body content for Patreon post {post_ref.post_id}: {keys}"
        )

    if paragraphs and _normalize_title_like(paragraphs[0]) == _normalize_title_like(
        title
    ):
...
async def _download_cover(fetcher: PatreonFetcher, meta: BookMeta) -> None:
    ...
    try:
        data, content_type = await asyncio.to_thread(
            _fetch_cover_direct, meta.cover_url
        )
    except Exception as exc:  # noqa: BLE001
