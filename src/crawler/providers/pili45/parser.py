#!/usr/bin/env python3
"""Collect and parse pili45 source material for the shared ingest workflow."""
...
from src.crawler.fetch.parallel import crawl_items, retry_async
...
def parse_info(
    html: str, cat_id: str, book_id: str, *, host: str = DEFAULT_HOST
) -> BookMeta:
...
def parse_chapter(html: str) -> tuple[str, list[str]]:
    ...
    content = soup.find("div", class_="read-content") or soup.find(
        class_="j_readContent"
    )
    paragraphs: list[str] = []
    if content:
...
def split_content_sections(chapters: list[Chapter]) -> list[Chapter]:
    ...
    for number, paragraphs in sections:
        if number in merged:
            print(
                f"[!] duplicate content section {number}; merging paragraphs",
                file=sys.stderr,
            )
    ...
    if missing:
        print(
            f"[!] missing content section number(s): {', '.join(map(str, missing))}",
            file=sys.stderr,
        )
...
def _resolve_book_url(arg: str) -> str:
    if arg.startswith("http"):
        ...
        if m:
            return (
                f"{parsed.scheme}://{parsed.netloc}/{m.group(1)}/{m.group(2)}/info.html"
            )
        return arg
    if "/" in arg:
        cat, bid = arg.split("/", 1)
