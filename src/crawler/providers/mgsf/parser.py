#!/usr/bin/env python3
"""Collect and parse mgsf source material for the shared ingest workflow."""
...
_CN_ORDINAL = (
    r"(?:[一二三四五六七八九]?十[一二三四五六七八九]?|百|[一二三四五六七八九])"
)
...
def parse_info(html: str, cat_id: str, book_id: str) -> BookMeta:
    ...
    if intro_p:
        for tag in intro_p.find_all(["script", "ins", "iframe", "style"]):
            tag.decompose()
        for c in intro_p.find_all(
            string=lambda t: t and isinstance(t, type(intro_p.string))
        ):
...
def parse_menu(
    html: str, cat_id: str, book_id: str
...
def crawl_book(book_url: str, *, delay: float = 0.4) -> tuple[BookMeta, list[Volume]]:
    ...
    for i, ref in enumerate(refs, 1):
        ...
        _chap_title_from_page, paragraphs = parse_chapter_html(
            fetcher.get_html(ref.url)
        )
