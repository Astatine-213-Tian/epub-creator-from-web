...
from src.crawler.fetch.browser import start_browser, wait_for_page_ready
...
def _get_search_html(url: str) -> str:
    ...
    try:
        ...
        PROGRESS.provider_detail(
            "mgsf", "raw search blocked; starting browser fallback"
        )
...
def preview_book(result: SearchResult) -> BookPreview:
    ...
    while menu_url:
        ...
        page_refs, next_url = parser.parse_menu(
            fetcher.get_html(menu_url), cat_id, book_id
        )
