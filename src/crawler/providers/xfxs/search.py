...
from src.crawler.fetch.browser import wait_for_page_ready
from src.crawler.search.engines import UA, WebSearchResult, site_search, web_search
...
def search_books(
    ...
    *,
    ...
    PROGRESS.provider_detail(
        "xfxs", "native search is unavailable; using external site search"
    )
...
async def _browser_external_search(
    query: str, *, browser, limit: int
    ...
    for engine, url in (
        ...
        (
            "google",
            f"{GOOGLE_SEARCH_URL}?q={quote_plus(query + ' xfxs1')}&hl=zh-CN&num={limit}",
        ),
    ):
...
def _external_items_from_html(html_text: str, *, engine: str) -> list[WebSearchResult]:
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "xfxs1.com" not in href and "xfxs1.com" not in title:
            continue
        items.append(
            WebSearchResult(title=title, url=href, description="", engine=engine)
        )
    return items
...
def _search_author_pages(
    ...
    *,
    ...
    authors = (
        ...
        else _discover_author_candidates(query)
    )
    if not authors:
        return []
...
def _books_from_author_page(author: str) -> list[tuple[str, str]]:
    ...
    try:
        response = requests.get(
            url, headers={"User-Agent": UA}, timeout=AUTHOR_PAGE_TIMEOUT
        )
