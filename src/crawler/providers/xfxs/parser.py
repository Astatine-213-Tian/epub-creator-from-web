#!/usr/bin/env python3
"""Collect and parse xfxs source material for the shared ingest workflow."""
...
def _trim_repeated_intro_prefix(
    chapter: Chapter, intro_paragraphs: list[str]
) -> list[str]:
...
def _drop_repeated_intro_chapter(volumes: list[Volume], meta: BookMeta) -> None:
    for volume in volumes:
        ...
        if _chapter_repeats_intro(first, meta.intro_paragraphs):
            ...
            if trimmed:
                print(
                    f"[+] trimming duplicated intro prefix: {first.title}",
                    file=sys.stderr,
                )
                first.paragraphs = trimmed
            else:
                print(
                    f"[+] dropping duplicated intro chapter: {first.title}",
                    file=sys.stderr,
                )
    ...
    while url and url not in visited:
        ...
        for attempt in range(5):
            try:
                ...
                paras, first_is_continuation, page_title, next_url = (
                    _extract_chapter_paragraphs(html)
                )
                if paras:
                    break
                if attempt == 4:
                ...
                retry_delay = (
                    5.0 * (attempt + 1)
                    if ("HTTP 429" in message or "HTTP 520" in message)
                    else 1.0 * (attempt + 1)
                )
                print(
    ...
    try:
        ...
        if meta.cover_url:
            try:
                ...
                print(
                    f"[+] cover {len(meta.cover_bytes)} bytes ({meta.cover_url})",
                    file=sys.stderr,
                )
