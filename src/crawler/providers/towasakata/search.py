...
def search_books(
    query: str, *, limit: int = 10, author: str | None = None
) -> list[SearchResult]:
...
def preview_book(result: SearchResult) -> BookPreview:
    ...
    chapter_titles = [
        cc.convert(chapter.title) for volume in volumes for chapter in volume.chapters
    ]
    return BookPreview(
        parser="towasakata",
        ...
        intro=BeautifulSoup(
            parser.render_intro_html(first.intro_html, cc), "lxml"
        ).get_text(" "),
...
def _is_catalog_result(title: str, snippet: str, url: str) -> bool:
    ...
    return (
        ...
        or "updated to" in normalized_snippet
        and "blog-entry-49" in url
    )
...
def _is_relevant_result(query: str, title: str, author: str) -> bool:
    ...
    if not normalized_query:
        return True
    return normalized_query in _normalize_search_text(
        title
