#!/usr/bin/env python3
"""Collect and parse zhenhun source material for the shared ingest workflow."""
    ...
    for line in lines:
        ...
        if marker:
            if current_number is not None and current_paragraphs:
                sections.append(
                    Section(current_number, current_paragraphs, ref.title, ref.url)
                )
...
def _sections_to_chapters(section_groups: list[list[Section]]) -> list[Chapter]:
    ...
    for section in sections:
        ...
        chapters.append(
            Chapter(title=f"第{section.number}节", paragraphs=section.paragraphs)
        )
...
def crawl_book(
    ...
    *,
    ...
    section_groups = asyncio.run(
        crawl_posts(refs, delay=delay, concurrency=concurrency)
    )
