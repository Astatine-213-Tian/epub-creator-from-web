...
from src.fetch.browser import resolve_browser_executable, wait_for_page_ready
...
def parse_detail_page(html: str, detail_url: str) -> BookMeta:
    ...
    file_size = download_text[len(extension_match.group(0)) :].lstrip(" ,") if extension_match else ""
    ...
    return BookMeta(
        ...
        extension=extension,
        file_size=file_size,
        description=_clean_text(_attr(description_node, "content")) if description_node else "",
...
async def _wait_for_download(page, directory: Path, *, timeout: float = 90.0) -> Path:
    ...
    while asyncio.get_running_loop().time() < deadline:
        ...
        files = [path for path in directory.iterdir() if path.is_file() and not path.name.endswith(".crdownload")]
        for path in files:
...
async def download_epub(
    ...
    *,
    ...
    try:
        ...
        await wait_for_page_ready(page, ready_selector="h1.book-title", settle_delay=delay)
        ...
        with tempfile.NamedTemporaryFile(suffix=".epub") as probe:
            ...
            with zipfile.ZipFile(probe.name) as archive:
                ...
                if bad_member:
                    raise RuntimeError(f"downloaded EPUB has a corrupt ZIP member: {bad_member}")
                if "mimetype" not in archive.namelist():
                    raise RuntimeError("downloaded ZIP is missing the EPUB mimetype member")
        return meta, data
    finally:
        await browser.stop()
