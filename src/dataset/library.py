"""Extract text, classify books and maintain targeted dataset manifests."""
...
from src.dataset.text import BLANK_RE, clean_text_line
from src.metadata.classifier import (
    classify_many_with_codex,
    classify_with_codex,
    metadata_to_cache,
)
from src.metadata.jjwxc import JjwxcTypeMetadata, classify_from_text, fetch_jjwxc_type
from src.runtime.paths import dataset_txt_output_path
...
def _is_content_spine_item(
    *,
    ...
    if "html" not in media_type and not lower_href.endswith(
        (".xhtml", ".html", ".htm")
    ):
        return False
...
def load_jjwxc_manifest(
    manifest_path: Path | None, top50_path: Path | None
...
def prefill_codex_cache(
    *,
    ...
    for epub_path in epub_paths:
        ...
        if (
            title in exclude_titles
            or _metadata_key(title, author) in exclude_metadata_keys
        ):
            continue
    ...
    *,
    ...
    for cache_key in _metadata_cache_keys(title, author):
        ...
        if cached:
            ...
            return JjwxcTypeMetadata(**cached), jjwxc_url
    if codex_classify:
        codex_metadata = classify_with_codex(
            title=title, author=author, sample=text[:6000]
        )
        if codex_metadata:
