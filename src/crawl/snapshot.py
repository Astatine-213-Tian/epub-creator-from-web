...
def load_manifest(snapshot_dir: Path) -> dict[str, Any]:
    ...
    if version != CRAWL_SCHEMA_VERSION:
        raise ValueError(f"unsupported crawl snapshot schema {version}: {manifest_path}")
    return data
...
def comment_path(snapshot_dir: Path, manifest: dict[str, Any], chapter_id: str) -> Path:
    for item in manifest.get("chapters", []):
        if str(item.get("id")) == chapter_id:
            return snapshot_dir / str(item.get("comments_path") or f"comments/{chapter_id}.json")
    raise KeyError(f"chapter id not in manifest: {chapter_id}")


def load_chapter(snapshot_dir: Path, manifest: dict[str, Any], chapter_id: str) -> dict[str, Any]:
    return load_json(chapter_path(snapshot_dir, manifest, chapter_id))


def load_comments(snapshot_dir: Path, manifest: dict[str, Any], chapter_id: str) -> list[dict[str, Any]]:
